...
class ReplayView:
    ...
    def render(self, surf: pygame.Surface, flow: ReplayFlow,
               frame: int = 0) -> None:
        ...
        if not flow.entries:
            self._text(surf, f, "(replays/ 下没有录像; 游戏内 Esc 暂停菜单"
                                " 选 Save Replay 录制)", 70, 120, _DIM)
        else:
            y = 96
            for i, e in enumerate(flow.entries[:16]):  # 首屏 16 条
                ...
                line = (f"No.{i + 1:>2}  {CHARACTERS[ch]:<8} "
        ...
        if frame % 60 < 45:  # 闪烁提示
            self._text(surf, f, "↑↓: 选择  Z: 播放 (播放中 Esc 退出)  X/Esc: 返回",
