...
本类是整合层: 各引擎模块(player/bomb/items/boss/enemies)为纯逻辑,
计数与状态收口在 engine/globals.ZunGlobals; 引擎透出的副作用事件
(PlayerEvent/BombStartResult/handle_timer_callback/end_spellcard 等)
由本类逐帧消费并接回 globals/items/bullets; 发声经 engine/sound.SoundQueue
...
from ..engine.boss import SPELLCARD_SCORE, Boss
from ..engine.globals import ZunGlobals
from ..engine.results import RunStats, ScoreRecord, TopList, clear_percent, rating
...
DEFAULT_SCORE_PATH = Path(__file__).resolve().parent.parent.parent / "score.json"

# CHARACTER_SHT(角色 → .sht 文件名)已集中于 touhou/games_th07.py(单一来源);
...
class PerfectCherryBloom:
    ...
    def __init__(self, data_path: str | Path | None = None, character: int = 0,
        ...
        # 数值表(registry.GameData; 缺省 = th07 表, games_th07.TH07_DATA)
        ...
        # ---- 音效/BGM 事件(引擎纯逻辑透出, 播放层每帧消费; engine/sound.py) ----
        ...
        # 状态(计数/分数/樱点/动态难度收口在 globals, 见 engine/globals.py)
        self.frame = 0
