...
消费方式(双向皆通, 不传也能跑):
- 引擎模块(boss/bomb/items)模块级同名常量 = 本模块的表(import 自这里),
  独立构造 Boss/Bomb/ItemWorld 时默认即 th07 数值;
- 经注册表: TH07_DATA 由 ``register_game_data("th07", ...)`` 登记,
  Game/TouhouWorld 构造对局时经 ``data=`` 注入(core/impl 的 data 参数),
  对局实现内按 ``self.data`` 读(sht 映射/符卡分值/掉落表已接线)。

新作品(th08): 照抄本模块结构出自己的 games_th08.py(换数值),
...
# 引擎侧(engine/bomb.py)包成 BombParams 后消费。
