...
通用核; th07 专属机制只住本文件, 经 ``@register_mods("th07")`` 登记,
``ModApi(game)`` 构造时实例化本提供者并把公开方法收割进 capabilities
能力表(契约见 touhou/registry.py 的 register_mods)。作品包内摸
``game._impl`` 是同层操作, 不算破封装。
"""
from __future__ import annotations

from typing import TYPE_CHECKING, cast

from ...registry import register_mods
