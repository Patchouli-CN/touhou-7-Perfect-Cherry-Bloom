...
- 每帧 (Bomb.tick): UpdateBombProjectiles(伤害盒 size.x 清零、清弹盒
  lifetime--/半径增长) → 每帧扣樱点(_tick_resource_cost hook, 透出
  drain_applied, 上层调 ZunGlobals.subtract_cherry_drain) → 机体 calc。
本模块留 th07 专属:
- 12 套炸弹按 (character, focus) 查表分派 (_BOMB_CALCS), 全部移植自 BombData.cpp。
- 樱点消耗 compute_bomb_cherry_drain() 与六机体参数表 BOMB_PARAMS。
...
from ...engine.bomb_base import (  # noqa: F401 (框架类型/函数为兼容再导出)
...
# 同包 data.py 的 BOMB_PARAMS —— 单一来源, 这里只包成 BombParams)
...
class BombContext(_BombContextBase):
    """bombCalc 的每帧外部输入(th07 扩展: 樱点/最后命中敌人位置)。

    通用字段(player_pos/difficulty/rng_float)在基类; rng_float 为 None 时
    各机体 calc 回落 random.random。
    """
...
class Bomb(BombBase[BombContext]):
    """一次炸弹的生命周期(th07) —— 通用生命周期/盒判定在基类 BombBase。

    上层每帧另消费 drain_applied(本帧应扣樱点, 调 ZunGlobals.subtract_cherry_drain)。
    """
