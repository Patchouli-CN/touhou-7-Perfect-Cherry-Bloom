...
class ContinueView:
    ...
    def render(self, surf: pygame.Surface, cursor: int, retries_left: int) -> None:
        ...
        for row, (sid, cx, cy) in enumerate(_LAYOUT):
            ...
            surf.blit(spr, spr.get_rect(center=(GAME_X + cx + dx, GAME_Y + cy + dy)))
        if 1 <= retries_left <= 5:
            ...
            if digit is not None:
                surf.blit(
                    digit,
                    digit.get_rect(
                        center=(GAME_X + _DIGIT_CENTER[0], GAME_Y + _DIGIT_CENTER[1])
                    ),
                )
