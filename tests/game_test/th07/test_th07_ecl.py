...
def _instr(
    time: int, op: int, args: tuple = (), mask: int = 0, skip: int = 0xFF
) -> bytes:
    size = 12 + 4 * len(args)
...
class RecordingHost(EclHost):
    ...
    def spawn_bullet_pattern(self, props) -> None:
        self.events.append(
            (
                ...
                props.count2,
                props.aim_mode,
                round(props.speed1, 4),
                round(props.speed2, 4),
                props.flags,
            )
        )
    ...
    def spawn_enemy(
        self, sub_id, pos, life, item_drop, score, mirror, context_args
    ) -> None:
        self.events.append(
            (
                "spawn_enemy",
...
def test_int_math_c_semantics() -> None:
    ...
    m, _, _ = make_machine(
        [
            ...
            _instr(
                0, OP.DIV, (V.LOCAL_INT1_3, V.LOCAL_INT1_1, V.LOCAL_INT1_2), mask=0b111
            ),
            # MOD: -7 % 2 = -1
            _instr(
                0, OP.MOD, (V.LOCAL_INT1_4, V.LOCAL_INT1_1, V.LOCAL_INT1_2), mask=0b111
            ),
...
def test_float_math_and_var_refs() -> None:
    ...
    m, _, _ = make_machine(
        [
            ...
            _instr(
                0, OP.SET_FLOAT, (vf(V.LOCAL_FLOAT1_3), vf(V.LOCAL_FLOAT1_1)), mask=0b11
            ),
            _instr(
                0,
                OP.MUL_FLOAT,
            ...
            _instr(
                0,
                OP.LERP,
                (vf(V.LOCAL_FLOAT1_5), vf(V.LOCAL_FLOAT1_2), _f(0.0), _f(0.25)),
                mask=0b011,
            ),
            _instr(0, OP.UNIMP),
        ]
...
def test_difficulty_skip() -> None:
    ...
    subs = [
        [
            _instr(
                0, OP.SET_INT, (V.LOCAL_INT1_1, 1), mask=0b01, skip=0b0001
            ),  # 仅 Easy
...
def test_spawn_bullet_pattern_outputs_shooter() -> None:
    ...
    m, host, _ = make_machine(
        [
            _instr(
                0,
                OP.SPAWN_BULLET_PATTERN_RING_AIMED,
...
def test_replay_real_sub2_callback_loop() -> None:
    ...
    assert (
        frames[1] == (True, []) and frames[2] == (True, []) and frames[3] == (True, [])
    )
