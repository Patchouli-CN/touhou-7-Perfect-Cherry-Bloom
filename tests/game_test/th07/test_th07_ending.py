    ...
    class _Stub:
        def __init__(self) -> None:
            self.ending = EndingData(
                character=0, bad=False, path="", segments=[], ops=[("end",)]
            )  # 首帧即 @z
            self.finished = False
...
def test_real_ending_and_staff_roll() -> None:
    ...
    while not p.done and frames < 60000:
        ...
        saw_ending_no = saw_ending_no or any("ＥＮＤＩＮＧ" in l.text for l in p.texts)
        frames += 1
...
def test_ending_view_render_smoke() -> None:
    ...
    staff = EndingData(
        ...
        path="staff00.end",
        segments=[],
        ops=parse_end_ops(archive.load("staff00.end")),
    )
    for f in range(900):
