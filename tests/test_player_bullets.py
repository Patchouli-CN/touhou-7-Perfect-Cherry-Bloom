...
from touhou.games.th07.player import (  # noqa: E402
    ...
    UPDATE_ORB_LASER, UPDATE_PLAYER_LASER, UPDATE_UPWARD_ACCEL,
    OPTION_ANGLE_CENTER,
)
...
def E(**kw) -> ShotEntry:
    ...
    d = dict(fire_interval=5, fire_offset=0, offset=(0.0, 0.0),
             hitbox=(12.0, 12.0), angle=-math.pi / 2, speed=12.0, damage=10,
             option=0, bullet_state2=0, fire_cb=0, update_cb=0, draw_cb=0,
             hit_cb=0)
    d.update(kw)
    return ShotEntry(**d)
...
def test_real_sht_callback_index_distribution() -> None:
    ...
    expect = {
        ...
        "ply00bs.sht": {"upd": set()},
        "ply01a.sht":  {"upd": {UPDATE_UPWARD_ACCEL}, "hit": {HIT_MISSILE}},  # 魔理沙A 导弹
        "ply01as.sht": {"upd": {UPDATE_UPWARD_ACCEL}, "hit": {HIT_MISSILE},
                        "fire": {0, FIRE_DEFAULT}},               # 含显式 default(1)
        "ply01b.sht":  {"fire": {0, FIRE_ORB_UNFOCUSED}, "upd": {0, UPDATE_ORB_LASER}},
        "ply01bs.sht": {"fire": {0, FIRE_ORB_FOCUSED}, "upd": {0, UPDATE_PLAYER_LASER},
                        "draw": {0, 1}},                          # 拖尾
        "ply02a.sht":  {"fire": {0}},
...
def test_real_sht_marisa_b_persistent_slots() -> None:
    ...
    orbs = [e for lv in sd.levels for e in lv.entries
    ...
    lasers = [e for lv in sdf.levels for e in lv.entries
...
def test_orb_unfocused_occupies_timer_slot() -> None:
    p = make_player(E(fire_cb=FIRE_ORB_UNFOCUSED, fire_interval=100, fire_offset=0,
                      option=1, update_cb=UPDATE_ORB_LASER, bullet_state2=4,
                      speed=0.0))
    fire_frames(p, 1)
    ts = p.timers[0]
...
def test_orb_requires_matching_option_state() -> None:
    ...
    p = make_player(E(fire_cb=FIRE_ORB_FOCUSED, fire_interval=4, fire_offset=2,
                      option=0, update_cb=UPDATE_PLAYER_LASER, bullet_state2=5,
                      draw_cb=1))
    fire_frames(p, 5)
    assert not live(p)
    # UNFOCUSED 型在 FOCUSED 时不发射
    p2 = make_player(E(fire_cb=FIRE_ORB_UNFOCUSED, fire_interval=100, fire_offset=0,
                       option=1, update_cb=UPDATE_ORB_LASER, bullet_state2=4))
...
def test_orb_entry_change_interrupts_old_bullet() -> None:
    e1 = E(fire_cb=FIRE_ORB_UNFOCUSED, fire_interval=100, fire_offset=0,
           option=1, update_cb=UPDATE_ORB_LASER, bullet_state2=4, speed=0.0)
    p = make_player(e1)
    fire_frames(p, 1)
    old = p.timers[0].bullet
    # 换 entry(同槽): 旧弹中断, 本帧不发射, 下一帧发新弹
    e2 = E(fire_cb=FIRE_ORB_UNFOCUSED, fire_interval=80, fire_offset=0,
           option=1, update_cb=UPDATE_ORB_LASER, bullet_state2=4, speed=0.0)
...
def test_orb_timer_expires_bullet_dies() -> None:
    p = make_player(E(fire_cb=FIRE_ORB_UNFOCUSED, fire_interval=10, fire_offset=0,
                      option=1, update_cb=UPDATE_ORB_LASER, bullet_state2=4, speed=0.0))
    fire_frames(p, 1)
    assert len(live(p)) == 1
...
def test_orb_laser_follows_option_geometry() -> None:
    p = make_player(E(fire_cb=FIRE_ORB_UNFOCUSED, fire_interval=100, fire_offset=0,
                      option=2, update_cb=UPDATE_ORB_LASER, bullet_state2=4, speed=0.0))
    fire_frames(p, 1)
    b = p.timers[0].bullet
...
def test_player_laser_follows_player_and_history_shifts() -> None:
    p = make_player(E(fire_cb=FIRE_ORB_FOCUSED, fire_interval=4, fire_offset=2,
                      option=0, update_cb=UPDATE_PLAYER_LASER, bullet_state2=5,
                      draw_cb=1, speed=0.0, offset=(0.0, -24.0)))
...
def test_rotating_orb_angle_includes_option_zero_entries() -> None:
    p = make_player(E(fire_cb=FIRE_ROTATING_ORB, angle=0.0, speed=12.0, option=0),
...
def test_missile_hit_transforms_then_decays_every_other_frame() -> None:
    p = make_player(E(hit_cb=HIT_MISSILE, bullet_state2=3, anm_file_idx=1090,
                      speed=0.0, damage=9))
