...
def test_handle_timer_callback_timeout_fails_capture() -> None:
    ...
    for _ in range(600):
        b.tick()
        ev = b.handle_timer_callback(
            cherry_above_start=40000, clear_field_cb=lambda: cleared.append(1)
        )
...
def test_stage_factor() -> None:
    assert [stage_factor(s) for s in (1, 2, 3, 4, 5, 6, 7)] == [2, 4, 6, 8, 10, 10, 10]
...
def test_settle_damage_spellcard_scaling() -> None:
    ...
    r = settle_damage(70, is_boss=True, is_focus=False, stage=1, spellcard_active=True)
    assert r.damage == 10
    r = settle_damage(5, is_boss=True, is_focus=False, stage=1, spellcard_active=True)
    assert r.damage == 1
...
def test_settle_damage_invincibility() -> None:
    # boss 无敌: /9
    r = settle_damage(63, is_boss=True, is_focus=True, stage=1, invincibility_timer=10)
    assert r.damage == 7
    # 非 boss 无敌: 0
    r = settle_damage(63, is_boss=False, is_focus=True, stage=1, invincibility_timer=10)
    assert r.damage == 0
...
def test_settle_damage_reimu_a_cherry() -> None:
    ...
    r = settle_damage(
        70, is_boss=False, is_focus=False, stage=1, enemy_timer=1, is_reimu_a=True
    )
    assert r.cherry_gain == 10
    r = settle_damage(
        70, is_boss=False, is_focus=False, stage=1, enemy_timer=2, is_reimu_a=True
    )
    assert r.cherry_gain == 20
    # 非 ReimuA 不减
    r = settle_damage(
        70, is_boss=False, is_focus=False, stage=1, enemy_timer=1, is_reimu_a=False
    )
    assert r.cherry_gain == 20
    # 30 同理: 100//28*10 = 30 → 奇帧 20
    r = settle_damage(
        100, is_boss=False, is_focus=False, stage=1, enemy_timer=3, is_reimu_a=True
    )
    assert r.cherry_gain == 20
    # 非 20/30 不动: 50//28*10 = 10
    r = settle_damage(
        50, is_boss=False, is_focus=False, stage=1, enemy_timer=1, is_reimu_a=True
    )
    assert r.cherry_gain == 10
...
def test_settle_damage_reimu_a_stage_reduction() -> None:
    ...
    r = settle_damage(
        40, is_boss=False, is_focus=False, stage=4, enemy_timer=2, is_reimu_a=True
    )
    assert r.damage == 28 and r.score_code == 50  # 28//5*10
    # 减免在 70 封顶之前: 100 → 100-25-6=69
    r = settle_damage(
        100, is_boss=False, is_focus=False, stage=4, enemy_timer=2, is_reimu_a=True
    )
    assert r.damage == 69
    # stage5/6 非 boss: 减半 (40 → 20, 100 → 50)
    for st in (5, 6):
        r = settle_damage(
            40, is_boss=False, is_focus=False, stage=st, enemy_timer=2, is_reimu_a=True
        )
        assert r.damage == 20
        r = settle_damage(
            100, is_boss=False, is_focus=False, stage=st, enemy_timer=2, is_reimu_a=True
        )
        assert r.damage == 50
    # stage7 不减
    r = settle_damage(
        40, is_boss=False, is_focus=False, stage=7, enemy_timer=2, is_reimu_a=True
    )
    assert r.damage == 40
    # boss 不减 (isBoss 排除)
    r = settle_damage(
        40, is_boss=True, is_focus=False, stage=5, enemy_timer=2, is_reimu_a=True
    )
    assert r.damage == 40
    # focus 的普通敌: 樱点块不进 → 不减
    r = settle_damage(
        40, is_boss=False, is_focus=True, stage=5, enemy_timer=2, is_reimu_a=True
    )
    assert r.damage == 40
    # bomb 中: 樱点块不进 → 不减
    r = settle_damage(
    ...
    # 非 ReimuA stage5 不减
    r = settle_damage(
        40, is_boss=False, is_focus=False, stage=5, enemy_timer=2, is_reimu_a=False
    )
    assert r.damage == 40
